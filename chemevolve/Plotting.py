...
def plot_length_distribution(filename, savename=None):
    '''Plots the time averaged length distribution of molecules in the entire system. Shows plot
        unless a savename is specified '''
...
def plot_molecule_distribution(filename, savename=None):
    '''Plots the time averaged molecule distribution of molecules in the entire system. Shows plot
        unless a savename is specified '''
    ts_df = pd.read_csv(filename)
    max_length = 0
    static_df = ts_df.filter(items=['molecule', 'abundance'])
    ...
    for i in range(1, max_length + 1):
        ...
        label = "length %i" % i
        plt.bar([x for x in range(current_x, current_x + len(indices_used))],
                current_molecules,
                yerr=current_std,
                color=color_palette[(i - 1)],
                ecolor=color_palette[(i - 1)],
                label=label)
...
def plot_time_series(filename, print_molcules, savename=None):
    '''Plot the time series of molecule names in print_molcules, if savename provide figure will be
        saved '''
    ...
    for i in range(len(print_molcules)):
        ...
        mass_fraction = [float(x) / 10000 for x in abundances]  # Why is 10000 in there?
        time = list(np.array(ts_df.index.values, dtype=float))
        z = zip(mass_fraction, time)
        z = sorted(z, key=lambda x: x[1])
        mass_fraction, time = zip(*z)
        [ci1] = random.sample(colorindex, 1)
        colorindex.remove(ci1)
        plt.plot(time, mass_fraction, color=color_palette[ci1])
